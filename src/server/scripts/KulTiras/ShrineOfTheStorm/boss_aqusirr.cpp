...
struct npc_aqualing : public AquasirrBase
{
    ...
    {
        AquasirrBase::Reset();
        me->SetFloatValue(UNIT_FIELD_BOUNDINGRADIUS, 3.84367f); // sniffed, using same model as boss, with smaller bounding
        me->SetFloatValue(UNIT_FIELD_COMBATREACH, 7.5f); // sniffed, using same model as boss, with smaller combatreach

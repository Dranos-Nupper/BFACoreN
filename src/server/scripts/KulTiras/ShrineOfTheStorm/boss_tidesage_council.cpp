        ...
        if (Creature* faye = instance->GetCreature(NPC_GALECALLER_FAYE))
        {
            if (me->GetHealth() <= damage)
            {
                if (faye->HasAura(SPELL_PERMANENT_FEIGN_DEATH))
                {
                    ...
                    faye->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                    faye->KillSelf();
    ...
    void DamageTaken(Unit* /*attacker*/, uint32& damage) override
    {
        if (Creature* ironhull = instance->GetCreature(NPC_BROTHER_IRONHULL))
        {
            if (me->GetHealth() <= damage)
            {
                ...
                {
                    // both bosses dead, loot
                    ironhull->RemoveFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE | UNIT_FLAG_NOT_ATTACKABLE_1);
                    ironhull->KillSelf();

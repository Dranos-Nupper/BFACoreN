...
uint8 ProtectorsAlive(InstanceScript* instance, Creature* me)
{
    ...
    Creature* asani = instance->instance->GetCreature(instance->GetObjectGuid(NPC_ANCIENT_ASANI));
    if (asani && asani->IsAlive())
        ++count;

    Creature* regail = instance->instance->GetCreature(instance->GetObjectGuid(NPC_ANCIENT_REGAIL));
    if (regail && regail->IsAlive())
        ++count;

    Creature* kaolan = instance->instance->GetCreature(instance->GetObjectGuid(NPC_PROTECTOR_KAOLAN));
        ...
        struct boss_ancient_regailAI : public BossAI
        {
			...
			void Reset()
			{
				...
				if (pInstance)
                {
                    ...
                    if (Creature* minionController = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_MINION_OF_FEAR_CONTROLLER)))
			...
			void JustDied(Unit* killer)
            {
                ...
                if (pInstance)
                {
                    ...
                    Creature* asani = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_ANCIENT_ASANI));
                    Creature* kaolan = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_PROTECTOR_KAOLAN));
            ...
            void DoAction(const int32 action)
            {
                switch (action)
                {
                    ...
                    case ACTION_INTRO_FINISHED:
                        ...
                        if (GameObject* vortex = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(GOB_COUNCILS_VORTEX)))
                            vortex->SetGoState(GO_STATE_ACTIVE);
                        if (GameObject* wall = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(GOB_WALL_OF_COUNCILS_VORTEX)))
        ...
        struct boss_ancient_asaniAI : public BossAI
        {
			...
			void JustDied(Unit* killer)
            {
                ...
                if (pInstance)
                {
                    ...
                    Creature* regail = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_ANCIENT_REGAIL));
                    Creature* kaolan = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_PROTECTOR_KAOLAN));
        ...
        struct boss_protector_kaolanAI : public BossAI
        {
			...
			void UpdateAI(const uint32 diff)
            {
                if (!IntroDone(pInstance, me))
                {
                    me->SetReactState(REACT_PASSIVE);
                    me->SetFlag(UNIT_FIELD_FLAGS, /*UNIT_FLAG_DISABLE_MOVE|*/UNIT_FLAG_NOT_SELECTABLE|UNIT_FLAG_NON_ATTACKABLE|UNIT_FLAG_IMMUNE_TO_PC|UNIT_FLAG_LOOTING);
                    me->AddUnitMovementFlag(UNIT_STATE_ROOT);
        ...
        struct mob_minion_of_fearAI : public ScriptedAI
        {
            ...
            void Reset()
            {
                if (pInstance && !protectorTargetedGuid)
                {
                    ...
                    if (Creature* asani = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_ANCIENT_ASANI)))
                        if (asani->IsAlive())
                            targets.push_back(asani);
                    if (Creature* kaolan = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_PROTECTOR_KAOLAN)))
                        if (kaolan->IsAlive())
                            targets.push_back(kaolan);
                    if (Creature* regail = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_ANCIENT_REGAIL)))
                        if (regail->IsAlive())
        ...
        class spell_corrupted_essence_SpellScript : public SpellScript
        {
            ...
            void CorrectRange(std::list<WorldObject*>& targets)
            {
                ...
                for (auto itr : targets)
                {
                    ...
                    if (Aura* corruptedEssence = itr->ToUnit()->GetAura(SPELL_CORRUPTED_ESSENCE))
                    {
            ...
            void CorrectTargets(std::list<WorldObject*>& targets)
            {
                ...
                /*for (WorldObject* target : targets)
                    if (Unit* unitTarget = target->ToUnit())
                        if (unitTarget->HasAura(SPELL_CLEANSING_WATERS_REGEN))

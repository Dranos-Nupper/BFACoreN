...
// "instance_terrace_of_endless_spring" 996

struct instance_terrace_of_endless_spring : public InstanceScript
{
    instance_terrace_of_endless_spring(InstanceMap* map) : InstanceScript(map) {}
    ...
    void Initialize()
    {
        ...
        tsulongEventTimer = ObjectGuid::Empty;
    ...
    bool SetBossState(uint32 id, EncounterState state)
    {
        ...
        if (id == DATA_PROTECTORS && state == DONE)
            if (Creature* c = instance->GetCreature(GetObjectGuid(NPC_TSULONG)))
...
{
    RegisterInstanceScript(instance_terrace_of_endless_spring, 996);
}

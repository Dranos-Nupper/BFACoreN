            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_CHECK_ENERGY:
                    {
                        ...
                        {
                            if (Creature* pureLight = ObjectAccessor::GetCreature(*me, pInstance->GetObjectGuid(NPC_PURE_LIGHT_TERRACE)))
            ...
            void CorrectRange(std::list<WorldObject*>& targets)
            {
                ...
                if (!players.isEmpty())
                    for (Map::PlayerList::const_iterator itr = players.begin(); itr != players.end(); ++itr)
                        if (Player* player = itr->GetSource())
                            if (!GetCaster()->isInFront(player, 3.14159f /*pi*/ / 3))
        ...
        class spell_penetrating_bolt_SpellScript : public SpellScript
        {
            ...
            void CorrectRange(std::list<WorldObject*>& targets)
            {
                ...
                for (auto itr : targets)
                    if (itr->GetGUID() != ObjectGuid::Empty)
            ...
            void HandleDummy(SpellEffIndex index)
            {
                if (Unit* caster = GetCaster())
                {
                    ...
                    {
                        if (Player* target = ObjectAccessor::GetPlayer(*caster, targetGuid))
                            //caster->CastSpell(target, SPELL_PENETRATING_BOLT_MISSILE, true, NULL, NULLAURA_EFFECT, instance->GetData64(NPC_SHA_OF_FEAR));
                            caster->CastSpell(target, SPELL_PENETRATING_BOLT_MISSILE, true);
                    }
                }
            }

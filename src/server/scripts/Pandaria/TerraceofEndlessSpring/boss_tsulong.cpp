                ...
                if (phase == PHASE_NIGHT)
                {
                    switch (events.ExecuteEvent())
                    {
                        ...
                        case EVENT_SPAWN_SUNBEAM:
                            me->SummonCreature(SUNBEAM_DUMMY_ENTRY, me->GetRandomNearPosition(30.0f));
                            break;
                        default:
                            break;
                    }
        ...
        struct npc_sunbeamAI : public CreatureAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (scale <= 1.0f)
                {
                    if (pInstance)
                    {
                        if (Creature* tsulong = pInstance->instance->GetCreature(pInstance->GetObjectGuid(NPC_TSULONG)))

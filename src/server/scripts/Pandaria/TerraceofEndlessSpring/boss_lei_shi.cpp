    ...
    public:
        ...
        struct boss_lei_shiAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (GameObject* vortex = me->GetMap()->GetGameObject(pInstance->GetObjectGuid(GOB_LEI_SHIS_VORTEX)))
                    vortex->SetGoState(GO_STATE_ACTIVE);

                if (GameObject* wall = me->GetMap()->GetGameObject(pInstance->GetObjectGuid(GOB_WALL_OF_LEI_SHI)))
            ...
            void DamageTaken(Unit* attacker, uint32& damage)
            {
                ...
                if (me->HealthBelowPctDamaged(endCombatPct, damage))
                {
                    ...
                    if (me->GetMap()->IsHeroic())
                        me->SummonGameObject(GOB_LEI_SHI_CHEST_HEROIC, leiShiPos.GetPositionX(), leiShiPos.GetPositionY(), leiShiPos.GetPositionZ(), 0, QuaternionData(), 0);
                    else
                        me->SummonGameObject(GOB_LEI_SHI_CHEST_NORMAL, leiShiPos.GetPositionX(), leiShiPos.GetPositionY(), leiShiPos.GetPositionZ(), 0, QuaternionData(), 0);
                }
            }
...
class mob_animated_protector : public CreatureScript
{
    public:
        ...
        struct mob_animated_protectorAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer)
            {
                if (Creature* leiShi = me->GetMap()->GetCreature(pInstance->GetObjectGuid(NPC_LEI_SHI)))
            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                switch (events.ExecuteEvent())
                {
                    case EVENT_HIDDEN_SPRAY:
                    {
                        ...
                        if (pInstance)
                            leiShiGuid = pInstance->GetObjectGuid(NPC_LEI_SHI);

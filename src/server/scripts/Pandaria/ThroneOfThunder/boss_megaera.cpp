...
class boss_megaera : public CreatureScript
{
    public:
            ...
            void JustDied(Unit* p_Killer)
            {
                ...
                switch (GetDifficulty())
                {
                    case DIFFICULTY_10_N:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_MEGAERA_CHEST_10_NORMAL), DAY);
                        break;
                    case DIFFICULTY_25_N:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_MEGAERA_CHEST_25_NORMAL), DAY);
                        break;
                    case DIFFICULTY_10_HC:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_MEGAERA_CHEST_10_HEROIC), DAY);
                        break;
                    case DIFFICULTY_25_HC:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_MEGAERA_CHEST_25_HEROIC), DAY);
                        break;
                }
            ...
            void SummonedCreatureDies(Creature* summon, Unit* killer)
            {
                if (IsHeadEntry(summon->GetEntry()))
                {
                    ...
                    if (m_DiedHeadsCount < 7)
                    {
                        ...
                        if (diedHeadIndex >= 0)
                            m_nearHeadGuids[diedHeadIndex] = ObjectGuid::Empty;
                ...
                switch (events.ExecuteEvent())
                {
                    ...
                    case EVENT_RESPAWN_HEADS_2:
                    {
                        ...
                        if (oldFarHeadIndex >= 0)
                        {
                            ...
                            m_farHeadGuids[oldFarHeadIndex] = ObjectGuid::Empty;
                        }
        ...
        private:
            ...
            ObjectGuid m_nearHeadGuids[4];
            ObjectGuid m_farHeadGuids[7];
        ...
        private:
            ...
            uint8 GetFreeFarHeadIndex()
            {
                for (uint8 i = 0; i < 7; ++i)
                {
                    if (m_farHeadGuids[i] == ObjectGuid::Empty)
                        return i;
                }
    ...
    public:
        ...
        struct npc_megaera_headAI : public Scripted_NoMovementAI
        {
            ...
            void EnterCombat(Unit* attacker)
            {
                ...
                if (pInstance)
                {
                    if (Creature* megaera = ObjectAccessor::GetCreature(*me, pInstance->GetObjectGuid(DATA_MEGAERA)))
                    {
            ...
            void PlaySoundAtSpawn()
            {
                ...
                for (Map::PlayerList::const_iterator itr = plrList.begin(); itr != plrList.end(); ++itr)
                {
                    if (Player* plr = itr->GetSource())
                        plr->PlayDirectSound(SOUND_HYDRA_EMERGE, plr);
                        //plr->(SOUND_HYDRA_EMERGE, true);
                }
            }

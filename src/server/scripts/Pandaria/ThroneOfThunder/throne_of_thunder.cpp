            ...
            void UpdateAI(const uint32 diff)
            {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        {
                            Player* player = me->SelectNearestPlayer(20.0f);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_LEI_SHEN_TELEPORT:
                            if (InstanceScript* pInstance = me->GetInstanceScript())
                            {
                                if (GameObject* pGo = ObjectAccessor::GetGameObject(*me, pInstance->GetObjectGuid(DATA_LIGHTNING_SPAN)))
                                {
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SLIME_TRAIL:
                        {
                            ...
                            std::vector<AreaTrigger*> triggers = me->GetAreaTriggers(SPELL_SLIME_TRAIL_AREATRIGGER);
                            bool canSpawn = true;
                            if (!triggers.empty())
                            {
                                for (std::vector<AreaTrigger*>::const_iterator itr = triggers.begin(); itr != triggers.end(); ++itr)
                                {
...
public:
    ...
    class spell_throne_of_thunder_spirit_light_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            players = GetCaster()->SelectNearestPlayers(19, true);
            //Trinity::AnyPlayerInObjectRangeCheck check(GetCaster(), 19.0f);
            //Trinity::PlayerListSearcher<JadeCore::AnyPlayerInObjectRangeCheck> searcher(GetCaster(), players, check);
...
void AddSC_throne_of_thunder()
{
    ...
    //todo check what an spellarreatriggerscript should be

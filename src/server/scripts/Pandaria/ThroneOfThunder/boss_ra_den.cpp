        ...
        private:
            ...
            void BuildCorruptedEssensesPositions(std::vector<Position> &positions, Position const& center, float radius, float z)
            {
                ...
                for (uint8 i = 0; i < 40; angle += step, ++i)
                {
                    ...
                    z = me->GetMap()->GetHeight(me->GetPhaseShift(), pos.m_positionX, pos.m_positionY, z);
                    pos.m_positionZ = z;
            ...
            void SpawnLoot()
            {
                switch (GetDifficulty())
                {
                    case DIFFICULTY_10_HC:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_CACHE_OF_STORMS_10_HEROIC), DAY);
                        break;
                    case DIFFICULTY_25_HC:
                        instance->DoRespawnGameObject(instance->GetObjectGuid(DATA_CACHE_OF_STORMS_25_HEROIC), DAY);
                        break;
                }
            }
        };
    ...
    public:
        ...
        private:
            ...
            Creature* GetRaden()
            {
                ...
                return pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_RA_DEN));
            }
    ...
    public:
        ...
        class spell_ra_den_unstable_vita_aura_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                ...
                Creature* pRaden = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_RA_DEN));
                if (!pRaden)
                    return;

...
struct npc_dark_animus_golem_baseAI : public ScriptedAI
{
    ...
    private:
        ...
        void ActivateGolem()
        {
            ...
            if (Player* target = me->SelectNearestPlayer(100.f))
                AttackStart(target);
        }
    ...
    public:
        ...
        private:
            ...
            void SpawnGolems()
            {
                ...
                // spawn anima golems
                std::list<ObjectGuid> animaGolems;
                ...
                if (!Is25ManRaid())
                {
                    Trinity::Containers::RandomResize(animaGolems, 13);
                    for (std::list<ObjectGuid>::const_iterator itr = animaGolems.begin(); itr != animaGolems.end(); ++itr)
                    {
                ...
                std::list<ObjectGuid> largeAnimaGolems;
                for (uint8 i = 0; i < 8; ++i)
                {
                ...
                if (!Is25ManRaid())
                {
                    Trinity::Containers::RandomResize(largeAnimaGolems, 3);
                    for (std::list<ObjectGuid>::const_iterator itr = largeAnimaGolems.begin(); itr != largeAnimaGolems.end(); ++itr)
                    {
                        if (Creature* pGolem = instance->instance->GetCreature((*itr)))
                        {
                            pGolem->AI()->DoAction(ACTION_SET_DAMAGED);
            ...
            void ActivateGolem()
            {
                ...
                if (Player* target = me->SelectNearestPlayer(50.f))
                    AttackStart(target);
            }
        ...
        struct npc_dark_animus_anima_orbAI : public Scripted_NoMovementAI
        {
            ...
            void DamageTaken(Unit* attacker, uint32& damage)
            {
                ...
                if (!isExploded)
                {
                    ...
                    if (pInstance)
                    {
                        if (Creature* pAnimus = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_DARK_ANIMUS)))
                        {
    ...
    public:
        ...
        struct npc_dark_animus_dark_ritualistAI : public ScriptedAI
        {
            ...
            void EnterCombat(Unit* p_Attacker)
            {
                if (m_Instance)
                {
                    if (Creature* l_AnimaOrb = ObjectAccessor::GetCreature(*me, m_Instance->GetObjectGuid(NPC_ANIMA_ORB)))
...
class npc_dark_animus_crimson_wake : public CreatureScript
{
    public:
        ...
        struct npc_dark_animus_crimson_wakeAI : public ScriptedAI
        {
            ...
            {
                isMoving = false;
                m_FollowedPlayerGUID = ObjectGuid::Empty;
            }
            ...
            {
                if (Unit* l_Target = me->SelectNearestPlayer(10.0f))
                {
        ...
        private:
            ...
            void BeginMoving()
            {
                if (!isMoving)
                {
                    ...
                    if (Unit* target = ObjectAccessor::GetUnit(*me, m_FollowedPlayerGUID))
                    {
        ...
        private:
            ...
            ObjectGuid m_FollowedPlayerGUID;
            bool isMoving;
        };
};
    ...
    public:
        ...
        class spell_dark_animus_crimson_wake_aoe_SpellScript : public SpellScript
        {
            ...
            void HandleHitTarget(SpellEffIndex effIndex)
            {
                ...
                GetHitUnit()->CastSpell(GetHitUnit(), SPELL_CRIMSON_WAKE, TRIGGERED_CAN_CAST_WHILE_CASTING_MASK);
            }
...
public:
    ...
    class spell_dark_animus_matter_swap_AuraScript : public AuraScript
    {
        ...
        void OnRemove(AuraEffect const* p_AurEff, AuraEffectHandleModes p_Mode)
        {
            ...
            Unit* target;
            std::list<Player*> players = owner->SelectNearestPlayers(100.0f, true);
            Trinity::Containers::RandomShuffle(players);

            for (Player* player : players)
            {
                if (!player->HasAura(SPELL_TARGETED_MATTER_SWAP))
                {
                    target = player;
                    break;
                }
            ...
            Position ownerPosition = owner->GetPosition(), targetPosition = target->GetPosition();
            ...
            if (damageToOwner)
            {
                GetCaster()->CastCustomSpell(owner, SPELL_MATTER_SWAP_DAMAGE, &damageToOwner, NULL, NULL, true);
            }

            if (damageToTarget)
            {
                GetCaster()->CastCustomSpell(target, SPELL_MATTER_SWAP_DAMAGE, &damageToTarget, NULL, NULL, true);
            }
...
void AddSC_boss_dark_animus()
{
    ...
    //new spell_dark_animus_matter_swap(); creates an algortihm error

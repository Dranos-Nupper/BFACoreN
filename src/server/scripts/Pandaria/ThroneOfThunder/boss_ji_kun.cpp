...
class boss_ji_kun : public CreatureScript
{
    public:
        ...
        struct boss_ji_kunAI : public BossAI
        {
            ...
            {
                std::list<Creature*> creatures = me->FindNearestCreatures(entry, 200.0f);
            ...
            void JustDied(Unit* p_Killer)
            {
                ...
                if (GameObject* l_ExitDoor = instance->instance->GetGameObject(instance->GetObjectGuid(GO_JI_KUN_EXIT_DOOR)))
        ...
        private:
            ...
            {
                std::list<Creature*> creaturelist = me->FindNearestCreatures(NPC_HATCHLING, 300.0f);
                for (Creature* hatchling : creaturelist)
                {
                    if (hatchling->AI()->GetData(TYPE_IS_HATCHLING_HUNGRY) == 0)
                        creatures.push_back(hatchling);
                }
            }
                ...
                switch (p_Action)
                {
                    case ACTION_ACTIVATE_NEST:
                    {
                        ...
                        for (uint8 i = 0; i < maxEggs; ++i)
                        {
                            ...
                            float ori  = frand(0.0f, 2 * 3.141592f);
                ...
                while (uint32 l_EventId = m_Events.ExecuteEvent())
                {
                    switch (l_EventId)
                    {
                        ...
                        case EVENT_FLIGHT:
                        {
                            ...
                            float l_Ori  = frand(0.0f, 2 * 3.141592f);
        ...
        struct npc_ji_kun_exit_chamberAI : public Scripted_NoMovementAI
        {
            ...
            void UpdateAI(uint32 const diff)
            {
                if (!GetClosestGameObjectWithEntry(me, GO_FEATHER_OF_JI_KUN, 10.0f))
                    //me->CastSpell(me, SPELL_DROP_FEATHERS_GOB, false);
                    me->SummonGameObject(GO_FEATHER_OF_JI_KUN, me->GetPositionX(), me->GetPositionY(), me->GetPositionZ(), 0.0f, QuaternionData(), 15000);
            }
        };
};
    ...
    public:
        ...
        struct go_ji_kun_feather_of_jikunAI : public GameObjectAI
        {
            ...
            bool GossipHello(Player* player, bool /*isUse*/)
            {
                ...
                {
                    if (player->HasAura(SPELL_FLIGHT))
                        return false;
                }
                else
                {
                ...
                go->CastSpell(player, SPELL_DAEDALIAN_WINGS);

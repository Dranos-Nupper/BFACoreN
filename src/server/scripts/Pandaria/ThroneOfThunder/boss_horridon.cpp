...
class HorridonAddsController
{
    ...
    private:
        ...
        void ActivateFarakkiWasteWalkers()
        {
            for (uint8 i = 0; i < 3; ++i)
            {
                if (Creature* pWalker = ObjectAccessor::GetCreature(*m_Owner, farrakiWalkerGUIDs[i]))
                {
                    pWalker->GetMotionMaster()->MoveJump(farakkiWasteWalkerPos[i + 3].GetPositionX(), farakkiWasteWalkerPos[i + 3].GetPositionY(), farakkiWasteWalkerPos[i + 3].GetPositionZ(), 0, 40.0f, 20.0f);
        ...
        void ActivateGurubashiPriests()
        {
            for (uint8 i = 0; i < 3; ++i)
            {
                if (Creature* pPriest = ObjectAccessor::GetCreature(*m_Owner, gurubashiPriestGUIDs[i]))
                {
                    pPriest->GetMotionMaster()->MoveJump(gurubashiPriestPos[i + 3].GetPositionX(), gurubashiPriestPos[i + 3].GetPositionY(), gurubashiPriestPos[i + 3].GetPositionZ(), 0, 40.0f, 20.0f);
        ...
        void ActivateDrakkariWarlords()
        {
            for (uint8 i = 0; i < 3; ++i)
            {
                if (Creature* pWarlord = ObjectAccessor::GetCreature(*m_Owner, drakkariWarlordGUIDs[i]))
                {
                    pWarlord->GetMotionMaster()->MoveJump(drakkariWarlordPos[i + 3].GetPositionX(), drakkariWarlordPos[i + 3].GetPositionY(), drakkariWarlordPos[i + 3].GetPositionZ(), 0, 40.0f, 20.0f);
        ...
        void ActivateAmaniWarbears()
        {
            for (uint8 i = 0; i < 3; ++i)
            {
                if (Creature* pWarbear = ObjectAccessor::GetCreature(*m_Owner, amaniWarbearGUIDs[i]))
                {
                    pWarbear->GetMotionMaster()->MoveJump(amaniWarBearPos[i + 3].GetPositionX(), amaniWarBearPos[i + 3].GetPositionY(), amaniWarBearPos[i + 3].GetPositionZ(), 0, 40.0f, 20.0f);
    ...
    private:
        ...
        Creature* m_Owner;
        ObjectGuid farrakiWalkerGUIDs[3];
        ObjectGuid gurubashiPriestGUIDs[3];
        ObjectGuid drakkariWarlordGUIDs[3];
        ObjectGuid amaniWarbearGUIDs[3];
        Phases m_phase;
        bool m_IsFirstSpawn;
};
    ...
    public:
        ...
        struct boss_horridonAI : public BossAI
        {
            boss_horridonAI(Creature* creature) : BossAI(creature, DATA_HORRIDON), m_AddsController(me)
            {
                ...
                warGodJalakGuid = ObjectGuid::Empty;
            }
        ...
        private:
            ...
            ObjectGuid dinomancerGUIDs[4];
            ObjectGuid warGodJalakGuid;
            bool isUnderControl;
        ...
        private:
            ...
            void ActivateDinomancer(Phases phase)
            {
                ...
                if (Creature* pDinomancer = ObjectAccessor::GetCreature(*me, dinomancerGUIDs[mobIndex]))
                {
                    pDinomancer->GetMotionMaster()->MoveJump(dinomancerPos[4].GetPositionX(), dinomancerPos[4].GetPositionY(), dinomancerPos[4].GetPositionZ(), 0, 40.0f, 20.0f);
            ...
            {
                DespawnCreatures(NPC_AMANI_SHI_BEAST_SHAMAN);
                DespawnCreatures(NPC_VENOMOUS_EFFUSION_HORR);
                DespawnCreatures(NPC_LIVING_POISON);
                DespawnCreatures(NPC_FROZEN_ORB_HORR);
                DespawnCreatures(NPC_LIGHTNING_NOVA_TOTEM);
                DespawnCreatures(NPC_DIREHORN_SPIRIT);

                DespawnGameobjects(GO_ORB_OF_CONTROL_FARRAKI);
                DespawnGameobjects(GO_ORB_OF_CONTROL_GURUBASHI);
                DespawnGameobjects(GO_ORB_OF_CONTROL_DRAKKARI);
                DespawnGameobjects(GO_ORB_OF_CONTROL_AMANI);
            }

            void DespawnCreatures(int id)
            {
                std::list<Creature*> creatureList = me->FindNearestCreatures(id, 200);
                for (Creature* creature : creatureList)
                    creature->ForcedDespawn();
            }

            void DespawnGameobjects(int id)
            {
                std::list<GameObject*> gameobjectList = me->FindNearestGameObjects(id, 200);
            ...
            void ActivateWarGodJalak()
            {
                ...
                if (Creature* pJalak = ObjectAccessor::GetCreature(*me, warGodJalakGuid))
                {
    ...
    public:
        ...
        struct npc_horridon_zandalari_dinomancerAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                {
                    if (Creature* pHorridon = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_HORRIDON)))
                    {
        ...
        private:
            ...
            void SpawnOrbofControl()
            {
                ...
                if (InstanceScript* pInstance = me->GetInstanceScript())
                {
                    if (Creature* pHorridon = ObjectAccessor::GetCreature(*me, instance->GetObjectGuid(DATA_HORRIDON)))
                    {
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_JALAK_JUMP:
                            me->GetMotionMaster()->MoveJump(midPos.GetPositionX(), midPos.GetPositionY(), midPos.GetPositionZ(), 0, 15.0f, 15.0f);
    ...
    bool OnTrigger(Player* pPlayer, const AreaTriggerEntry* /*pAt*/, bool p_Enter)
    {
        if (InstanceScript* pInstance = pPlayer->GetInstanceScript())
        {
            ...
            if (pInstance->GetData(DATA_HORRIDON_INTRO) == NOT_STARTED)
            {
                ...
                if (Creature* pHorridon = ObjectAccessor::GetCreature(*pPlayer, pInstance->GetObjectGuid(DATA_HORRIDON)))
                {

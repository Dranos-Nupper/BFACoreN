    ...
    public:
        ...
        struct boss_iron_qonAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                m_RoshakGUID = ObjectGuid::Empty;
                m_QuetzalGUID = ObjectGuid::Empty;
                m_DamrenGUID = ObjectGuid::Empty;
                m_Phase = PHASE_NONE;
        ...
        private:
            ...
            ObjectGuid m_RoshakGUID;
            ObjectGuid m_QuetzalGUID;
            ObjectGuid m_DamrenGUID;
            Phases m_Phase;
    ...
    public:
        ...
        private:
            ...
            void SetBossVehicle()
            {
                if (pInstance)
                {
                    if (Creature* pIronQon = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_IRON_QON)))
                    {
    ...
    public:
        ...
        private:
            ...
            bool DeadZoneDamage(Unit* attacker)
            {
                ...
                {
                    bool isInFront = attacker->isInBack(me, 3.141592f / 2);
                    bool isInRight = HasInRight(attacker, 3.141592f / 2);
                    return isInFront || isInRight;
                }
                else if (lastDeadZoneSpell == SPELL_DEAD_ZONE_LEFT_RIGHT)
                {
                    bool isInLeft = HasInLeft(attacker, 3.141592f / 2);
                    bool isInRight = HasInRight(attacker, 3.141592f / 2);
                ...
                {
                    bool isInFront = attacker->isInBack(me, 3.141592f / 2);
                    bool isInBack = me->isInBack(attacker, 3.141592f / 2);
            ...
            bool HasInLeft(const Position* obj, float arc) const
            {
                ...
                float lborder = -1 * (arc/2.0f) + (3.141592f / 2);
                float rborder = (arc/2.0f) + (3.141592f / 2);
    ...
    public:
        ...
        struct npc_iron_qon_spear_2AI : public Scripted_NoMovementAI
        {
            npc_iron_qon_spear_2AI(Creature* creature) : Scripted_NoMovementAI(creature)
            {
                ...
                spawnTimer = 200;
                currentTick = 0;
                m_IronQonGuid = ObjectGuid::Empty;
            ...
            void Reset()
            {
                if (InstanceScript* pInstance = me->GetInstanceScript())
                    m_IronQonGuid = pInstance->GetObjectGuid(DATA_IRON_QON);
            }
            ...
            void UpdateAI(const uint32 diff)
            {
                if (canSpawnTriggers)
                {
                    if (spawnTimer <= diff)
                    {
                        ...
                        Creature* pIronQon = ObjectAccessor::GetCreature(*me, m_IronQonGuid);
                        if (!pIronQon)
                        {
                            canSpawnTriggers = false;
                            return;
                        ...
                        for (uint8 i = 0; i < 4; ++i)
                        {
                            Position pos = me->GetNearPosition(currentTick * 3, (float)i * (3.141592f / 2.0f));;
...
class spell_iron_qon_ligntning_storm: public SpellScriptLoader
{
    public:
        ...
        class spell_iron_qon_ligntning_storm_AuraScript : public AuraScript
        {
            ...
            /*void OnAbsorb(AuraEffect* aurEff, int32& remainingHeal, uint32& absorbAmount)
            {
    ...
    public:
        ...
        class spell_iron_qon_frozen_resilience_AuraScript : public AuraScript
        {
            ...
            void OnProc(AuraEffect const* aurEff, ProcEventInfo& eventInfo)
            {
                ...
                GetUnitOwner()->CastCustomSpell(attacker, SPELL_FROZEN_RESILIENCE_DMG, &damage, NULL, NULL, true);
            }

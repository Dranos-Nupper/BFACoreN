...
public:
    ...
    struct boss_lulinAI : public BossAI
    {
        ...
        void Reset()
        {
            ...
            if (Creature* pSuen = instance->instance->GetCreature(instance->GetObjectGuid(DATA_SUEN)))
            {
                if (!pSuen->IsAlive())
                {
                    pSuen->Respawn();
    ...
    public:
        ...
        struct boss_suenAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                if (Creature* pLulin = instance->instance->GetCreature(instance->GetObjectGuid(DATA_LULIN)))
                {
                    if (!pLulin->IsAlive())
                    {
                        pLulin->Respawn();
...
class npc_lulin_star_1 : public CreatureScript
{
    public:
        ...
        private:
            ...
            {
                std::list<Creature*> starList = me->FindNearestCreatures(NPC_STAR, 200);

                for (Creature* star : starList)
                    if (star->AI()->GetData(DATA_STAR_READY) == 0)
                        return star;
    ...
    public:
        ...
        class spell_lulin_tidal_force_AuraScript : public AuraScript
        {
            ...
            void HandleTick(AuraEffect const* aurEff)
            {
                ...
                Position pos;
                GetUnitOwner()->GetNearPoint2D(pos.m_positionX, pos.m_positionX, 80.0f, frand(0, 2 * 3.14159f));

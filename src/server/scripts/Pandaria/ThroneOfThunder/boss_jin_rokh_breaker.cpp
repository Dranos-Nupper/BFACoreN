...
struct StatueData
{
    public:
        ...
        StatueData(ObjectGuid triggerGuid, ObjectGuid fontainGuid, Position throwDest, Position waterDest)
        {
        ...
        ObjectGuid GetTriggerGuid() const
        {
            return m_TriggerGuid;
        }

        ObjectGuid GetFontainGuid() const
        {
...
class StatueController
{
    ...
    public:
        ...
        {
            m_Owner = owner;
            m_LastActivatedStatue = ObjectGuid::Empty;
        }
        ...
        void SpawnStatues()
        {
            ...
            for (uint8 i = 0; i < MAX_MOGU_STATUES; i++)
            {
                ...
                {
                    ObjectGuid fontainGuid = m_Owner->GetInstanceScript()->GetObjectGuid(statueFontainDataList[i]);
        ...
        void DespawnStatues()
        {
            ...
            {
                if (Creature* pStatueTrigger = ObjectAccessor::GetCreature(*m_Owner, (*itr)->GetTriggerGuid()))
                {
                    pStatueTrigger->DespawnOrUnsummon();
                }

                if (GameObject* pFontain = ObjectAccessor::GetGameObject(*m_Owner, (*itr)->GetFontainGuid()))
                {
        ...
        void ActivateNextStatueFor(Unit* target)
        {
            ...
            Creature* pStatueTrigger = ObjectAccessor::GetCreature(*m_Owner, statue->GetTriggerGuid());
            if (pStatueTrigger)
            {
            ...
            if (GameObject* pFontain = ObjectAccessor::GetGameObject(*m_Owner, statue->GetFontainGuid()))
            {
                pFontain->SetGoState(GO_STATE_ACTIVE);
            }

            // Spell doesn't work. Throw player at statue.
            target->GetMotionMaster()->MoveJump(statue->GetThrowDest().GetPositionX(), statue->GetThrowDest().GetPositionY(), 0, statue->GetThrowDest().GetPositionZ(), 25.0f, 25.0f);
        ...
        {
            return ObjectAccessor::GetCreature(*m_Owner, m_LastActivatedStatue);
        }
    ...
    private:
        ...
        ObjectGuid m_LastActivatedStatue;
};
    ...
    public:
        ...
        struct boss_jinrokh_the_breakerAI : public BossAI
        {
            ...
            void Reset()
            {
                ...
                std::list<Creature*> creatureList = me->FindNearestCreatures(NPC_FOCUSED_LIGHTNING, 200);
                std::list<Creature*> creatureList2 = me->FindNearestCreatures(NPC_LIGHTNING_FISSURE, 200);
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_LIGHTNING_STORM_JUMP:
                            ...
                            me->GetMotionMaster()->MoveJump(middlePos.GetPositionX(), middlePos.GetPositionY(), middlePos.GetPositionZ(), 0, 15.0f, 15.0f);
        ...
        private:
            ...
            {
                thunderingThrowTargetGuid = target ? target->GetGUID() : ObjectGuid::Empty;
            }

            Unit* GetThunderingThrowTarget() const
            {
                return ObjectAccessor::GetUnit(*me, thunderingThrowTargetGuid);
            }

        private:

            StatueController m_StatueController;
            ObjectGuid thunderingThrowTargetGuid;
            bool introDone;
        };
};
...
class npc_jinrokh_the_breaker_conductive_water : public CreatureScript
{
    public:
        ...
        struct npc_jinrokh_the_breaker_conductive_water_AI : public ScriptedAI
        {
            ...
            {
                me->SetFlag(UNIT_FIELD_FLAGS, UNIT_FLAG_NON_ATTACKABLE | UNIT_FLAG_NOT_SELECTABLE /*| UNIT_FLAG_DISABLE_MOVE*/);
                me->AddUnitState(UNIT_STATE_ALL_STATE);
    ...
    public:
        ...
        struct npc_jinrokh_the_breaker_focused_lightningAI : public ScriptedAI
        {
            ...
            void IsSummonedBy(Unit* summoner)
            {
                ...
                {
                    summoner->ToCreature()->AI()->Talk(ANN_FOCUSED_LIGHTNING);
        ...
        private:
            ...
            /*void CheckAchievement()
            {
                ...
                Trinity::CreatureLastSearcher<FocusedLightningCheck> searcher(me, creature, checker);
...
// Static Wound 138349. spell_jinrokh_the_breaker_static_wound
class spell_jinrokh_the_breaker_static_wound : public AuraScript
{
    PrepareAuraScript(spell_jinrokh_the_breaker_static_wound);
    ...
    void HandleProc(AuraEffect const* aurEff, ProcEventInfo & eventInfo)
    {
        ...
        GetUnitOwner()->CastCustomSpell(GetUnitOwner(), SPELL_STATIC_WOUND_DMG, &damage, NULL, NULL, true);
    }
    ...
    void Register()
    {
        OnEffectProc += AuraEffectProcFn(spell_jinrokh_the_breaker_static_wound::HandleProc, EFFECT_0, SPELL_AURA_DUMMY);
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_jinrokh_the_breaker_static_wound::HandlePeriodic, EFFECT_1, SPELL_AURA_PERIODIC_DUMMY);
    }
};
    ...
    public:
            ...
            class TanksCheck
            {
                public:
                    ...
                    bool operator()(WorldObject* unit) const
                    {
                        ...
                        if (unit->ToPlayer()->GetRoleForGroup() == ROLE_TANK)
                            return true;
        ...
        class spell_jinrokh_the_breaker_ionization_AuraScript : public AuraScript
        {
            ...
            void OnRemove(AuraEffect const* aurEff, AuraEffectHandleModes /*mode*/)
            {
                ...
                GetCaster()->CastCustomSpell(GetUnitOwner(), SPELL_IONIZATION_DMG, &damage, NULL, NULL, true);

                if (GetUnitOwner()->HasAura(SPELL_CONDUCTIVE_WATER))
                {
                    GetUnitOwner()->CastCustomSpell(GetUnitOwner(), SPELL_IONIZATION_CONDUCTION, &damage, NULL, NULL, true);
                }
            }
...
{
    public:
        WaterGuidCheck(ObjectGuid waterGuid) : m_WaterGuid(waterGuid) {}
...
public:
    ...
    class spell_jinrokh_the_breaker_focused_lightning_conduction_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            ObjectGuid waterGuid = pLightning->AI()->GetGUID(GUID_WATER);
            if (!waterGuid)
            {
                targets.clear();
                return;
...
public:
    ...
    class spell_jinrokh_the_breaker_ionization_conduction_SpellScript : public SpellScript
    {
        ...
        void FilterTargets(std::list<WorldObject*>& targets)
        {
            ...
            ObjectGuid waterGuid = aurWater->GetCasterGUID();
            if (!waterGuid)
            {
                targets.clear();
                return;
...
void AddSC_boss_jinrokh_the_breaker()
{
    ...
    RegisterAuraScript(spell_jinrokh_the_breaker_static_wound);

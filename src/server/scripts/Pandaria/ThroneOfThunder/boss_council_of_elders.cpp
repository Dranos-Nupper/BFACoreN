...
class boss_spirit_of_gara_jal_council : public CreatureScript
{
    public:
        ...
        struct boss_spirit_of_gara_jal_councilAI : public BossAI
        {
            ...
            void DespawnCreaturesInArea(int entry, Unit* me)
            {
                std::list<Creature*> creatures = me->FindNearestCreatures(entry, 200);
        ...
        private:
            ...
            Creature* GetBoss(uint32 type)
            {
                switch (type)
                {
                    ...
                    case DATA_FROST_KING_MALAKK:
                        return ObjectAccessor::GetCreature(*me, instance->GetObjectGuid(type));
                }
                return NULL;
            }
            ...
            Creature* SelectBossToPosses()
            {
                ...
                if (bosses.size() > 1)
                    bosses.sort(Trinity::HealthPctOrderPred(false));
    ...
    public:
        ...
        struct boss_kazra_jinAI : public ScriptedAI
        {
            ...
            void JustDied(Unit* killer)
            {
                ...
                if (pInstance)
                {
                    if (Creature* Garajal = ObjectAccessor::GetCreature(*me, pInstance->GetObjectGuid(DATA_GARAZHAL_THE_SPIRITBINDER)))
                    {
            ...
            void DamageTaken(Unit* doneBy, uint32 &uiDamage)
            {
                if (me->HasAura(SPELL_POSSESS))
                {
                    ...
                    {
                        const SpellInfo* overloadSpell = sSpellMgr->GetSpellInfo(SPELL_OVERLOAD_REFLECT_DMG);
                    ...
                    {
                        const SpellInfo* dischargeSpell = sSpellMgr->GetSpellInfo(SPELL_DISCHARGE_REFLECT_DMG);
            ...
            void UpdateAI(uint32 const diff)
            {
                ...
                if (possesed)
                {
                    if (possesedDamagePct - me->GetHealthPct() >= 25.0f)
                    {
                        ...
                        if (Creature* pGarazhal = ObjectAccessor::GetCreature(*me, pInstance->GetObjectGuid(DATA_GARAZHAL_THE_SPIRITBINDER)))
            ...
            Creature* GetBoss(uint32 type)
            {
                if (pInstance)
                {
                    switch (type)
                    {
                        ...
                        case DATA_HIGH_PRIESTESS_MARLI:
                        case DATA_FROST_KING_MALAKK:
                            return pInstance->instance->GetCreature(pInstance->GetObjectGuid(type));
                    }
                }
                return NULL;
            }
                ...
                while (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        case EVENT_LOA_SPIRIT_JUMP_KILL:
                            if (!healed && councilorTarget)
                                me->GetMotionMaster()->MoveJump(councilorTarget->GetPositionX(), councilorTarget->GetPositionY(), councilorTarget->GetPositionZ(), 0, 15.0f, 15.0f);
                            break;
...
// Possessed 136442
class spell_garajal_councilor_possessed : public AuraScript
{
    PrepareAuraScript(spell_garajal_councilor_possessed);
    ...
    void Register()
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_garajal_councilor_possessed::OnTick, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};
...
class spell_malakk_frigid_assault : public AuraScript
{
    PrepareAuraScript(spell_malakk_frigid_assault);
    ...
    void Register()
    {
        OnEffectPeriodic += AuraEffectPeriodicFn(spell_malakk_frigid_assault::OnPeriodic, EFFECT_0, SPELL_AURA_PERIODIC_DUMMY);
    }
};
    ...
    public:
        ...
        class spell_kazrajin_reckless_charge_force_SpellScript : public SpellScript
        {
            ...
            void HandleHit(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    Position pos = pKazrajin->GetNearPosition(i, 0.0f);
    ...
    public:
        ...
        class spell_kazrajin_reckless_charge_dummy_SpellScript : public SpellScript
        {
            ...
            void HandleHit(SpellEffIndex /*effIndex*/)
            {
                ...
                {
                    Position pos = GetCaster()->GetNearPosition(i, 0.0f);
...
void AddSC_boss_council_of_elders()
{
    ...
    RegisterAuraScript(spell_garajal_councilor_possessed);
    new spell_council_soul_fragment_transfer();
    RegisterAuraScript(spell_malakk_frigid_assault);

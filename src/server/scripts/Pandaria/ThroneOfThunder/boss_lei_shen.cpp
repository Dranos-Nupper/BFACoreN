...
class boss_lei_shen : public CreatureScript
{
    public:
                ...
                if (uint32 eventId = events.ExecuteEvent())
                {
                    switch (eventId)
                    {
                        ...
                        case EVENT_SUPERCHARGE_CONDUITS:
                            ...
                            if (!(diffusingChainConduitGUID != ObjectGuid::Empty))
                            {
                                events.ScheduleEvent(EVENT_DIFFUSION_CHAIN, 6000);
                            }

                            if (overchargeConduitGUID != ObjectGuid::Empty)
                            {
                                events.ScheduleEvent(EVENT_OVERCHARGE, 6000);
                            }

                            if (bouncingBoltConduitGUID != ObjectGuid::Empty)
                            {
                                events.ScheduleEvent(EVENT_BOUNCING_BOLT, 14000);
                            }

                            if (staticShockConduitGUID != ObjectGuid::Empty)
                            {
        ...
        private:
            ...
            ObjectGuid diffusingChainConduitGUID;
        ...
        private:
            ...
            void DespawnConduit(Conduits conduit, bool overcharge)
            {
                ...
                switch (conduit)
                {
                case OVERCHARGE_CONDUIT: overchargeConduitGUID = ObjectGuid::Empty; break;
                    case STATIC_SHOCK_CONDUIT: staticShockConduitGUID = ObjectGuid::Empty; break;
                    case DIFFUSION_CHAIN_CONDUIT: diffusingChainConduitGUID = ObjectGuid::Empty; break;
                    case BOUNCING_BOLT_CONDUIT: bouncingBoltConduitGUID = ObjectGuid::Empty; break;
                }
            }
            ...
            Creature* GetConduit(Conduits conduit)
            {
                switch (conduit)
                {
                    case BOUNCING_BOLT_CONDUIT: return ObjectAccessor::GetCreature(*me, bouncingBoltConduitGUID);
                    case OVERCHARGE_CONDUIT: return ObjectAccessor::GetCreature(*me, overchargeConduitGUID);
                    case STATIC_SHOCK_CONDUIT: return ObjectAccessor::GetCreature(*me, staticShockConduitGUID);
                    case DIFFUSION_CHAIN_CONDUIT: return ObjectAccessor::GetCreature(*me, diffusingChainConduitGUID);
                }
            ...
            {
                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_CHARGING_STATION)))
                {
            ...
            {
                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_FLOOR_NORTH)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_FLOOR_EAST)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_FLOOR_WEST)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }

                if (GameObject* pFloor = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_FLOOR_SOUTH)))
                {
                    pFloor->SetGoState(activate ? GO_STATE_ACTIVE : GO_STATE_READY);
                }
            }
            ...
            void ActivateRandomWindow()
            {
                ...
                uint32 spellId = 0;
                ObjectGuid windowGuid = ObjectGuid::Empty;
                switch (windSide)
                {
                    case 0:
                        spellId = SPELL_VIOLENT_GALE_WINDS_AREATRIGGER_1;
                        windowGuid = instance->GetObjectGuid(DATA_WINDOW_NORTH_WEST);
                        break;
                    case 1:
                        spellId = SPELL_VIOLENT_GALE_WINDS_AREATRIGGER_2;
                        windowGuid = instance->GetObjectGuid(DATA_WINDOW_SOUTH_WEST);
                        break;
                    case 2:
                        spellId = SPELL_VIOLENT_GALE_WINDS_AREATRIGGER_3;
                        windowGuid = instance->GetObjectGuid(DATA_WINDOW_NORTH_EAST);
                        break;
                    case 3:
                        spellId = SPELL_VIOLENT_GALE_WINDS_AREATRIGGER_4;
                        windowGuid = instance->GetObjectGuid(DATA_WINDOW_SOUTH_EAST);
                        break;
                }

                if (spellId && windowGuid != ObjectGuid::Empty)
                {
            ...
            {
                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_WINDOW_NORTH_WEST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_WINDOW_SOUTH_WEST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_WINDOW_NORTH_EAST)))
                    pGo->SetGoState(GO_STATE_READY);

                if (GameObject* pGo = instance->instance->GetGameObject(instance->GetObjectGuid(DATA_WINDOW_SOUTH_EAST)))
                    pGo->SetGoState(GO_STATE_READY);
            }
    ...
    public:
        ...
        private:
            ...
            void ActivateFloor(bool activate)
            {
                ...
                {
                    case NPC_STATIC_SHOCK_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(DATA_FLOOR_NORTH));
                        break;
                    case NPC_BOUNCING_BOLT_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(DATA_FLOOR_WEST));
                        break;
                    case NPC_OVERCHARGE_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(DATA_FLOOR_SOUTH));
                        break;
                    case NPC_DIFFUSION_CHAIN_CONDUIT:
                        pFloor = pInstance->instance->GetGameObject(pInstance->GetObjectGuid(DATA_FLOOR_EAST));
                        break;
                }
    ...
    public:
        ...
        struct npc_lei_shen_thunderous_throwAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* owner)
            {
                if (InstanceScript* pInstance = me->GetInstanceScript())
                {
                    if (Creature* pCreature = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_LEI_SHEN)))
                    {
        ...
        struct npc_lei_shen_lightning_whipAI : public Scripted_NoMovementAI
        {
            ...
            void IsSummonedBy(Unit* owner)
            {
                if (InstanceScript* pInstance = me->GetInstanceScript())
                {
                    if (Creature* pCreature = pInstance->instance->GetCreature(pInstance->GetObjectGuid(DATA_LEI_SHEN)))
                    {
                        pCreature->SetFloatValue(0, pCreature->GetAngle(me));
